  ...
  try {
    ...
    typeDetail.innerHTML = `
      <div class="max-w-5xl mx-auto my-8 px-6 py-10  transform transition-all duration-500 animate-fadeIn">
        ...
        <!-- Image with gradient border and hover effect -->
        <div class="flex justify-center mb-8">
          <div class="relative p-1 w-full max-w-xl rounded-2xl overflow-hidden  border-transparent bg-gradient-to-r from-red-400  to-red-600">
            <img src="${type.image}" 
                 class="w-full h-90 object-cover rounded-xl duration-500 transform hover:scale-105">
          </div>
        </div>
        ...
        <div class="flex justify-center space-x-4">
          <a href="product-detail.html?id=${productId}" 
             class="bg-gray-900 text-white font-semibold px-8 py-3 rounded-full transition-all duration-300 hover:from-indigo-700 hover:to-purple-700 hover:shadow-lg transform hover:-translate-y-1">
    ...
    product.types.forEach((type, index) => {
      ...
      typeDiv.innerHTML = `
        ...
        <div>
          ...
          <input type="text" id="type-name-${index}" class="w-full p-2 border rounded" value="${
        type.name
      }">
        </div>
        <div>
          <label for="type-description-${index}" class="block">Type Description</label>
          <textarea id="type-description-${index}" class="w-full p-2 border rounded">${
        type.description
      }</textarea>
        </div>
        <div>
          <label for="type-price-${index}" class="block">Type Price</label>
          <input type="number" id="type-price-${index}" class="w-full p-2 border rounded" value="${
        type.price
      }">
        </div>
        <div>
          <label for="type-image-${index}" class="block">Type Image URL</label>
          <input type="text" id="type-image-${index}" class="w-full p-2 border rounded" value="${
        type.image
      }">
        </div>
      `;
...
document
  ...
  ?.addEventListener("submit", async (e) => {
    ...
    const types = Array.from(
      document.querySelectorAll("#types-container > div")
    ).map((typeDiv, index) => {
      return {
